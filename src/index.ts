...
const port = parseInt(process.env.PORT) || 25000;
...
const resolvers = {
  Mutation: {
      signUp: async (_source: any, {  email, name,user_type  }: any) => {
      ...
      signIn: async (_source: any, { email}: any) => {
          const [user] = await User.find({
              where: {
                email,
              },
...
const authorizationMiddleware = async (requestContext: { contextValue?: any; operation?: any; document?: any; }) => {
